function out = KK21(x,k,B,G,s,r)
    ...
    xi = ((k+1)./4).*(n(1,:).*m(1,:) - i.*x);
    eta = ((k+1)./4).*(n(2,:).*m(2,:) - i.*x);
    ...
    K21 = (xi.*d - eta.*c)./(a.*d - b.*c);
%     out = K21;
%     check = (K21.*exp(i.*x.*(s-r)) - 1/2.*sin((s-r).*x));
    ...
    xiy = ((k+1)./4).*(ny(1,:).*my(1,:) - i.*y);
    etay = ((k+1)./4).*(ny(2,:).*my(2,:) - i.*y);
    ...
    K21y = (xiy.*dy - etay.*cy)./(ay.*dy - by.*cy);
    
    out = (K21.*exp(i.*x.*(s-r)) + K21y.*exp(i.*y.*(s-r)) - sin((s-r).*x));
