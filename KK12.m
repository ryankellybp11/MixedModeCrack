function out = KK12(x,k,B,G,s,r)
    ...
    K12 = (eta.*a - xi.*b)./(a.*d - b.*c);
%     out = K12;
%     out = (K12.*exp(i.*x.*(s-r)) - sin((s-r).*x));
    ...
    K12y = (etay.*ay - xiy.*by)./(ay.*dy - by.*cy);
    
    out = (K12.*exp(i.*x.*(s-r)) + K12y.*exp(i.*y.*(s-r)) - sin((s-r).*x));
